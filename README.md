# RandomStatus
## English
A plugin that automatically and randomly changes the status of all heroines (personality, relationship, lewdness, etc.) at each in-game time period.  
...
* Personality Count: Configurable total personality count to support game expansions (e.g., Extension version = 44).

---

## 日本語
ゲーム内の時間帯ごとに、全ヒロインのステータス（性格、関係性、H度など）を自動でランダム変動させるプラグインです。  
常に新鮮なキャラクターを楽しめます。  
変化は恒常的なものです。  
...
* H度に応じて「淫乱」状態を自動付与。
* H度変動に対して特定の性格にだけ倍率を適用。
* 「行為拒否解除」「レズ・オナニー願望」の一括最大化。
  
### カスタマイズ:
* 「Plugin settings」より各項目のON/OFFおよび変動比率のスライダー調整が可能。
* ゲーム拡張に対応するため合計性格数設定。（エクステンション版は44かな？）
