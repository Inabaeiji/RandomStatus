...
namespace KKS_RandomStatus
{
    ...
    {
        public const string Guid = "com.inaba178.kks.randomstatus";
        public const string Name = "KKS_RandomStatus";
        public const string Version = "1.0.0";
    }

    [BepInPlugin(AppInfo.Guid, AppInfo.Name, AppInfo.Version)]
    public class KKS_RandomStatus : BaseUnityPlugin
    {
        ...
        static ConfigEntry<bool> randomVisitFlag;
        static ConfigEntry<int> visitMyRoom;
        ...
        void Awake()
        {
            ...
            charaPersonality = Config.Bind("3.Randomize the personality", "Number of varieties", 39,
                new ConfigDescription("Number of varieties of personality (extension is 44?)", null, new ConfigurationManagerAttributes { Order = 99 }));
            allOkFlag = Config.Bind("4.Unlock all rejections, Maximize lesbian & masturbation desires", "Enabled Unlock  rejections & misc.", true,
                new ConfigDescription("Enabled", null, new ConfigurationManagerAttributes { Order = 100 }));
            randomVisitFlag = Config.Bind("5.Randomize the desire to visit my room", "Enabled random visit desire", true,
                new ConfigDescription("Enabled", null, new ConfigurationManagerAttributes { Order = 100 }));
            visitMyRoom = Config.Bind("5.Randomize the desire to visit my room", "minimum value (x10)", 10,
                new ConfigDescription("Set the value value to this amount or higher", new AcceptableValueRange<int>(1, 10), new ConfigurationManagerAttributes { Order = 99 }));

            var harmony = new Harmony(AppInfo.Guid);
            var mMapMove = typeof(Cycle).GetMethod("MapMove", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);

            if (mMapMove != null)
            {
                harmony.Patch(mMapMove, prefix: new HarmonyMethod(typeof(KKS_RandomStatus), nameof(MapMovePrefix)));
            }
            else
            {
                UnityEngine.Debug.LogError("【RandomStatus】Not found: MapMove()");
            }
        }

        static void MapMovePrefix(Cycle __instance)
        {
            UnityEngine.Debug.Log($"【RandomStatus】Start of time-based processing: {__instance.nowType}");

            //「活動中（Morning〜Night）」判定以外はスキップ
            if (__instance == null || !__instance.isAction)
            {
                return;
            }
            ...
            {
                UnityEngine.Debug.Log("Processing was skipped because [Game.Instance.HeroineList] is not available.");
            }
        ...
        static void AllGirlRandomizer()
        {
            var actionScene = SingletonInitializer<ActionScene>.instance;

            if (actionScene == null)
            {
                UnityEngine.Debug.LogError("[RandomStatus] actionScene is null");
            }
            ...
            if (string.IsNullOrWhiteSpace(hDownPersonality.Value))
            {
                specPersonality = Array.Empty<int>();
            }
            else
            {
            ...
            foreach (var h in Game.HeroineList)
            {
                ...
                if (randomPersonalityFlag.Value)
                {
                    ...
                    h.charFile.parameter.personality = randomPersonality;
                ...
                if (randomVisitFlag.Value)
                {
                    h.isDayH = false;
                    h.myRoomCount = Mathf.Max(1, h.myRoomCount);
                    int randomVisit = UnityEngine.Random.Range(visitMyRoom.Value, 11);
                    h.visitDesire = randomVisit * 10;
                }
            }

            UnityEngine.Debug.Log($"Made {countF} friends");
            UnityEngine.Debug.Log($"Made {countL} lovers");
            UnityEngine.Debug.Log($"Made {countFL} full lovers");
            UnityEngine.Debug.Log($"Made {countH} horny");
            UnityEngine.Debug.Log("(Including characters on standby)");
        }
